...
xepOnline.Formatter = {
  ...
  */
  __format: function(ElementIDs, options) {
    ...
    this.__postBackFailure = options.onError || this.__postBackFailure
    this.__postBackSuccessCustom = options.onSuccess || function(){return null;}
  ...
  __postBackSuccess: function(Response) {
    xepOnline.Formatter.__postBackSuccessCustom()
    ...
    if (current_mimetype == 'image/svg+xml'){
      if(jQuery(xepOnline.Formatter.__container).attr('data-xeponline-embed-pending') === 'true'){
        ...
        if(jQuery(xepOnline.Formatter.__container).attr('data-xeponline-embed-pending') === 'true'){
          ...
          var win = window.open("","_blank","titlebar=yes");
          win.name = xepOnline.Formatter.filename;
          win.document.title = xepOnline.Formatter.filename;
          win.document.write('<html><head>');
          win.document.write('</head><body height="100%"><div id="target" style="height:100%;overflow-y:auto">');
          win.document.write(objbuilder);
          win.document.write('</div></body></html>');
